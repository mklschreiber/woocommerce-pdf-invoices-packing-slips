...
class Frontend {
	...
	function __construct()	{
		...
		add_filter( 'woocommerce_my_account_my_orders_actions', array( $this, 'my_account_deposit_pdf_link' ), 10, 2 );
		add_filter( 'woocommerce_api_order_response', array( $this, 'woocommerce_api_invoice_number' ), 10, 2 );
		add_filter( 'woocommerce_api_order_response', array( $this, 'woocommerce_api_deposit_invoice_number' ), 10, 2 );
	}
	...
	/**
	 * Display deposit download link on My Account page
	 */
	public function my_account_deposit_pdf_link( $actions, $order ) {
		$deposit_invoice = wcpdf_get_deposit_invoice( $order );
		if ( $deposit_invoice && $deposit_invoice->is_enabled() ) {
			$pdf_url = wp_nonce_url( admin_url( 'admin-ajax.php?action=generate_wpo_wcpdf&document_type=deposit_invoice&order_ids=' . WCX_Order::get_id( $order ) . '&my-account'), 'generate_wpo_wcpdf' );

			// check my account button settings
			$button_setting = $deposit_invoice->get_setting('my_account_buttons', 'available');
			switch ($button_setting) {
				case 'available':
					$deposit_invoice_allowed = $deposit_invoice->exists();
					break;
				case 'always':
					$deposit_invoice_allowed = true;
					break;
				case 'never':
					$deposit_invoice_allowed = false;
					break;
				case 'custom':
					$allowed_statuses = $button_setting = $deposit_invoice->get_setting('my_account_restrict', array());
					if ( !empty( $allowed_statuses ) && in_array( WCX_Order::get_status( $order ), array_keys( $allowed_statuses ) ) ) {
						$deposit_invoice_allowed = true;
					} else {
						$deposit_invoice_allowed = false;
			...
			// Check if invoice has been created already or if status allows download (filter your own array of allowed statuses)
			if ( $deposit_invoice_allowed || in_array(WCX_Order::get_status( $order ), apply_filters( 'wpo_wcpdf_myaccount_allowed_order_statuses', array() ) ) ) {
				$document_title = array_filter( $deposit_invoice->get_setting( 'title', array() ) );
				if ( !empty($document_title) ) {
					$button_text = sprintf ( __( 'Download %s (PDF)', 'woocommerce-pdf-invoices-packing-slips' ), $deposit_invoice->get_title() );
				} else {
					$button_text = __( 'Download deposit invoice (PDF)', 'woocommerce-pdf-invoices-packing-slips' );
				}
				$actions['deposit_invoice'] = array(
					'url'  => $pdf_url,
					'name' => apply_filters( 'wpo_wcpdf_myaccount_button_text', $button_text, $deposit_invoice )
				);
			}
		}
	...
	/**
	 * Add deposit invoice number to WC REST API
	 */
	public function woocommerce_api_deposit_invoice_number ( $data, $order ) {
		$data['wpo_wcpdf_deposit_invoice_number'] = '';
		if ( $deposit_invoice = wcpdf_get_deposit_invoice( $order ) ) {
			$deposit_invoice_number = $deposit_invoice->get_number();
			if ( !empty( $deposit_invoice_number ) ) {
				$data['wpo_wcpdf_deposit_invoice_number'] = $deposit_invoice_number->get_formatted();
			}
		}
